...
(function () {
  ...
  function addLineItem(values) {
    var item = values || { description: '', quantity: 1, unitPrice: '', category: 'maintenance' };
    var row = document.createElement('div'); row.className = 'invoice-line-item';
    row.innerHTML = '<input class="invoice-line-description" maxlength="500" placeholder="Description" value="' + escapeHtml(item.description) + '"><select class="invoice-line-category-select" aria-label="Spend category"><option value="maintenance"' + (item.category === 'maintenance' ? ' selected' : '') + '>Maintenance</option><option value="materials"' + (item.category === 'materials' ? ' selected' : '') + '>Materials</option><option value="planting"' + (item.category === 'planting' ? ' selected' : '') + '>Planting</option><option value="project"' + (item.category === 'project' ? ' selected' : '') + '>Project work</option><option value="other"' + (item.category === 'other' ? ' selected' : '') + '>Other</option></select><input class="invoice-line-quantity" type="number" min="0.01" step="0.01" aria-label="Quantity" value="' + escapeHtml(item.quantity) + '"><input class="invoice-line-price" type="number" min="0" step="0.01" aria-label="Unit price in pounds" placeholder="Unit price" value="' + escapeHtml(item.unitPrice) + '"><button class="invoice-remove-line" type="button" aria-label="Remove line">×</button>';
  ...
  async function createInvoice() {
    var lineItems = Array.prototype.map.call(document.querySelectorAll('.invoice-line-item'), function (row) { return { description: row.querySelector('.invoice-line-description').value, category: row.querySelector('.invoice-line-category-select').value, quantity: row.querySelector('.invoice-line-quantity').value, unitPrice: row.querySelector('.invoice-line-price').value }; });
  ...
  async function openInvoice(id) {
    try {
      ...
      var invoice = data.invoice; var items = data.items || []; var payments = data.payments || []; var paymentIntents = data.paymentIntents || [];
      var itemRows = items.map(function (item) { return '<tr><td>' + escapeHtml(item.description) + (item.category ? '<small class="invoice-line-category">' + escapeHtml({ maintenance: 'Maintenance', materials: 'Materials', planting: 'Planting', project: 'Project work', other: 'Other' }[item.category] || item.category) + '</small>' : '') + '</td><td class="amount">' + item.quantity + '</td><td class="amount">' + money(item.unit_price) + '</td><td class="amount">' + money(item.line_total) + '</td></tr>'; }).join('');
      ...
      var intentRows = paymentIntents.length ? '<div class="invoice-intents"><h4>Client payment signals</h4>' + paymentIntents.map(function (intent) { return '<article><strong>' + escapeHtml(intent.intentType === 'bank_transfer_notified' ? 'Transfer notification' : 'Payment-plan request') + '</strong><span>' + escapeHtml(intent.status) + '</span><p>' + escapeHtml(intent.note || '') + '</p><small>' + (intent.amount ? money(intent.amount) : '') + (intent.proposedDate ? ' · ' + escapeHtml(date(intent.proposedDate)) : '') + '</small>' + (intent.status === 'requested' ? '<div><button class="btn-portal btn-primary btn-compact" type="button" data-intent-action="acknowledged" data-intent-id="' + intent.id + '">Acknowledge</button><button class="btn-portal btn-ghost btn-compact" type="button" data-intent-action="cancelled" data-intent-id="' + intent.id + '">Close</button></div>' : '') + '</article>'; }).join('') + '</div>' : '';
      ...
      document.querySelectorAll('[data-intent-action]').forEach(function (button) { button.addEventListener('click', function () { resolvePaymentIntent(invoice.id, Number(button.dataset.intentId), button.dataset.intentAction); }); });
  ...
  async function resolvePaymentIntent(invoiceId, intentId, status) {
    try {
      await getJson('/api/admin-invoices', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: 'resolve-payment-intent', intentId: intentId, status: status }) });
      await loadInvoices(); openInvoice(invoiceId); showAlert(status === 'acknowledged' ? 'Client payment signal acknowledged. Record an actual payment only after it is received.' : 'Client payment signal closed.', 'success');
    } catch (error) { showAlert(error.message || 'Unable to update client payment signal.', 'error'); }
  }
